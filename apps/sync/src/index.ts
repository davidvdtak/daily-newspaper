...
export const handler = async (event: S3Event) => {
  ...
  let items = await api.listItems(true);
  ...
  if (!folder) {
    const folderRef = await api.putFolder(folderName);
    items = await api.listItems(true);
    folder = items.find((x) => x.id === folderRef.id) ||
  ...
  if (!existing) {
    await api.putPdf(visibleName, pdf, { parent: folder.id, refresh: true });
  }
  ...
  if (old.length) await Promise.all(old.map((x) => api.move(toItemRef(x), "trash", true)));
...
function toItemRef(entry: Entry) {
